...
struct IssueSplitView: View {
    ...
    var body: some View {
        TabView(selection: $store.state.selectedTab) {
                ...
                NavigationStack(path: $store.state.issueRouter.path) {
                    if let selectedIssue = store.state.issueRouter.selectedIssue {
                        ...
                        VStack(alignment: .leading) {
                            Text(Bundle.Strings.chooseIssuePlaceholder)
                                .font(.title2)
                                .fontWeight(.medium)
                                .foregroundColor(.secondary)
                            Text(Bundle.Strings.chooseIssueSubheading)
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    }
            ...
            .tabItem({ Label(Bundle.Strings.tabTopics, systemImage: "phone.bubble.fill" ) })
            .tag("topics")
            ...
            InboxView()
                .tabItem({ Label(Bundle.Strings.tabReps, systemImage: "person.crop.circle.fill.badge.checkmark") })
                .tag("inbox")
